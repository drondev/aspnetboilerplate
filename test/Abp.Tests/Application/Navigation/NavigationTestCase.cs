...
namespace Abp.Tests.Application.Navigation
{
    internal class NavigationTestCase
    {
        ...
        private static IPermissionChecker CreateMockPermissionChecker()
        {
            ...
            permissionChecker.IsGrantedAsync(new UserIdentifier(GuidStatics.ID_1, GuidStatics.ID_1), "Abp.Zero.UserManagement").Returns(Task.FromResult(true));
            permissionChecker.IsGrantedAsync(new UserIdentifier(GuidStatics.ID_1, GuidStatics.ID_1), "Abp.Zero.RoleManagement").Returns(Task.FromResult(false));
