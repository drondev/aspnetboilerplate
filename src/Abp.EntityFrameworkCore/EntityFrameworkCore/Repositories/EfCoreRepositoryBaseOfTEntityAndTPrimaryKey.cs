...
namespace Abp.EntityFrameworkCore.Repositories
{
        ...
        where TDbContext : DbContext
    {
        ...
        {
            await (await GetContextAsync()).AddRangeAsync(entities);
            if (entities.Any(r => r.IsTransient()))
            {
                await (await GetContextAsync()).SaveChangesAsync(CancellationTokenProvider.Token);
            }
        }
