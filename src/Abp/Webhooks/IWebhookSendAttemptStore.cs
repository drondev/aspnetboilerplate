...
namespace Abp.Webhooks
{
    public interface IWebhookSendAttemptStore
    {
        ...
        /// Checks is there any successful webhook attempt in last <paramref name="searchCount"/> items. Should return true if there are not X number items
        /// </summary>
        bool HasXConsecutiveFail(Guid? tenantId, Guid subscriptionId, int searchCount);

        Task<IPagedResult<WebhookSendAttempt>> GetAllSendAttemptsBySubscriptionAsPagedListAsync(Guid? tenantId, Guid subscriptionId, int maxResultCount, int skipCount);

        IPagedResult<WebhookSendAttempt> GetAllSendAttemptsBySubscriptionAsPagedList(Guid? tenantId, Guid subscriptionId, int maxResultCount, int skipCount);
